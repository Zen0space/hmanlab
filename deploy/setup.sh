...
# HmanLab Universal VPS Setup Script
# Usage: curl -fsSL https://hmanlab.com/setup.sh | sh
#    or: curl -fsSL https://hmanlab.com/setup.sh | sh -s -- --docker
...
usage() {
  cat <<EOF
${BOLD}HmanLab VPS Setup${RESET}

Usage:
  curl -fsSL https://hmanlab.com/setup.sh | sh
  curl -fsSL https://hmanlab.com/setup.sh | sh -s -- --docker
  bash deploy/setup.sh [OPTIONS]

Options:
  --docker      Use Docker instead of native binary
  --uninstall   Remove HmanLab from this system
...
do_uninstall() {
  ...
  ok "HmanLab has been uninstalled"
  exit 0
}
...
main() {
  ...
  printf "\n${BOLD}HmanLab Setup${RESET} (%s mode)\n\n" "$MODE"
