---
name: skill-creator
description: Create or update HmanLab skills. Use when designing, structuring, or authoring new agent skills.
metadata: {"hmanlab":{"emoji":"🛠️","requires":{}}}
---

# Skill Creator

Guide users through designing and building HmanLab skills. Follow these six steps in order — do not skip ahead.
...
```bash
hmanlab skills create <skill-name>
```

This creates `~/.hmanlab/skills/<skill-name>/SKILL.md` with a starter template.
...
metadata: {"hmanlab":{"emoji":"📦","requires":{"bins":["curl"],"any_bins":[],"env":[]},"install":[],"always":false,"os":[]}}
---
```

**HmanLab metadata fields:**
...
5. Run `hmanlab skills show <name>` to verify it loads correctly
6. Run `hmanlab skills list` to confirm it appears
