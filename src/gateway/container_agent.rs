...
const CONTAINER_WORKSPACE_DIR: &str = "/data/.hmanlab/workspace";
const CONTAINER_SESSIONS_DIR: &str = "/data/.hmanlab/sessions";
const CONTAINER_CONFIG_PATH: &str = "/data/.hmanlab/config.json";

/// Path inside the container where the env file is mounted (Apple Container only).
#[cfg(target_os = "macos")]
const CONTAINER_ENV_DIR: &str = "/tmp/hmanlab-env";
...
impl ContainerAgentProxy {
                            ...
                            if let Some(ref pairing_mutex) = self.pairing {
                                ...
                                if !valid {
                                    ...
                                    let rejection = OutboundMessage::new(
                                        &inbound.channel,
                                        &inbound.chat_id,
                                        "Access denied: device not paired. Use `hmanlab pair new` to generate a pairing code.",
                                    );
                                    if let Err(e) = self.bus.publish_outbound(rejection).await {
    ...
    async fn spawn_container(&self, request: &AgentRequest) -> Result<AgentResponse> {
        let config_root = dirs::home_dir().unwrap_or_default().join(".hmanlab");
    ...
    fn collect_env_vars(&self) -> Vec<(String, String)> {
        ...
        if let Some(ref anthropic) = self.config.providers.anthropic {
            if let Some(ref key) = anthropic.api_key {
                if !key.trim().is_empty() {
                    env_vars.push((
                        "HMANLAB_PROVIDERS_ANTHROPIC_API_KEY".to_string(),
                        key.clone(),
                    ));
                }
            }
            if let Some(ref base) = anthropic.api_base {
                if !base.trim().is_empty() {
                    env_vars.push((
                        "HMANLAB_PROVIDERS_ANTHROPIC_API_BASE".to_string(),
                        base.clone(),
                    ));
                }
            }
        ...
        if let Some(ref openai) = self.config.providers.openai {
            if let Some(ref key) = openai.api_key {
                if !key.trim().is_empty() {
                    env_vars.push(("HMANLAB_PROVIDERS_OPENAI_API_KEY".to_string(), key.clone()));
                }
            }
            if let Some(ref base) = openai.api_base {
                if !base.trim().is_empty() {
                    env_vars.push((
                        "HMANLAB_PROVIDERS_OPENAI_API_BASE".to_string(),
                        base.clone(),
                    ));
                }
            }
        ...
        if let Some(ref openrouter) = self.config.providers.openrouter {
            if let Some(ref key) = openrouter.api_key {
                if !key.trim().is_empty() {
                    env_vars.push((
                        "HMANLAB_PROVIDERS_OPENROUTER_API_KEY".to_string(),
                        key.clone(),
                    ));
                }
            }
            if let Some(ref base) = openrouter.api_base {
                if !base.trim().is_empty() {
                    env_vars.push((
                        "HMANLAB_PROVIDERS_OPENROUTER_API_BASE".to_string(),
                        base.clone(),
                    ));
                }
            }
        ...
        env_vars.push((
            "HMANLAB_AGENTS_DEFAULTS_WORKSPACE".to_string(),
    ...
    async fn build_apple_invocation(
        ...
        let container_name = format!("hmanlab-{}", Uuid::new_v4());
        let mut args = vec![
            "run".to_string(),
            "--rm".to_string(),
            "-i".to_string(),
        ...
        // Wrap command: source env file then exec hmanlab
        args.push("sh".to_string());
        args.push("-c".to_string());
        args.push(format!(
            ". {}/env.sh && exec hmanlab agent-stdin",
            CONTAINER_ENV_DIR
        ));
...
mod tests {
    ...
    fn test_build_docker_invocation_mounts_expected_paths_and_hides_secrets() {
        ...
        config.container_agent.image = "hmanlab:test".to_string();
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-proxy-test-{}", Uuid::new_v4()));
        ...
        assert!(invocation.env.iter().any(|(name, value)| {
            name == "HMANLAB_PROVIDERS_ANTHROPIC_API_KEY" && value == "secret-anthropic-key"
        }));
    ...
    fn test_collect_env_vars_includes_internal_paths() {
        ...
        assert!(
            vars.iter()
                .any(|(k, v)| k == "HMANLAB_AGENTS_DEFAULTS_WORKSPACE"
    ...
    fn test_build_docker_invocation_rejects_temp_binary() {
        ...
        let temp_root =
            std::env::temp_dir().join(format!("hmanlab-binary-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
    ...
    fn test_build_docker_invocation_accepts_well_known_binaries() {
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-wk-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
...
"#,
    ...
    fn test_build_docker_invocation_rejects_sensitive_extra_mount() {
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-mount-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
    ...
    fn test_build_docker_invocation_rejects_env_file_mount() {
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-env-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
    ...
    fn test_build_docker_invocation_rejects_aws_credentials_mount() {
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-aws-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
    ...
    fn test_build_docker_invocation_rejects_traversal_in_extra_mount() {
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-trav-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
    ...
    fn test_build_docker_invocation_accepts_safe_extra_mount() {
        ...
        let mount_root =
            std::env::temp_dir().join(format!("hmanlab-safe-mount-{}", Uuid::new_v4()));
        ...
        let temp_root = std::env::temp_dir().join(format!("hmanlab-safe-test-{}", Uuid::new_v4()));
        let workspace_dir = temp_root.join("workspace");
        let sessions_dir = temp_root.join("sessions");
        let config_path = temp_root.join("config.json");
        std::fs::create_dir_all(&workspace_dir).unwrap();
