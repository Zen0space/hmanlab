...
//!
//! ```rust,ignore
//! use hmanlab::providers::{openai::OpenAIProvider, ChatOptions, LLMProvider};
...
/// Can be overridden at compile time with `HMANLAB_OPENAI_DEFAULT_MODEL` env var.
const DEFAULT_MODEL: &str = match option_env!("HMANLAB_OPENAI_DEFAULT_MODEL") {
    Some(v) => v,
    None => "gpt-5.1",
};
...
/// Convert HmanLab messages to OpenAI API format.
...
/// Convert HmanLab tool definitions to OpenAI API format.
...
/// Convert OpenAI API response to HmanLab LLMResponse.
