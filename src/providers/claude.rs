...
//!
//! ```rust,ignore
//! use hmanlab::providers::{claude::ClaudeProvider, ChatOptions, LLMProvider};
...
/// Can be overridden at compile time with `HMANLAB_CLAUDE_DEFAULT_MODEL` env var.
const DEFAULT_MODEL: &str = match option_env!("HMANLAB_CLAUDE_DEFAULT_MODEL") {
    Some(v) => v,
...
impl ClaudeProvider {
    ...
    /// # Example
    /// ```
    /// use hmanlab::providers::claude::ClaudeProvider;
    /// use hmanlab::providers::LLMProvider;
    ///
...
/// Convert HmanLab messages to Claude API format.
///
...
/// Convert HmanLab tool definitions to Claude API format.
...
/// Convert Claude API response to HmanLab LLMResponse.
...
/// Convert HmanLab ToolCall to LLMToolCall.
///
