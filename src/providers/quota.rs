...
//!
//! ```rust
//! use hmanlab::providers::quota::{QuotaConfig, QuotaStore, QuotaCheckResult, QuotaPeriod};
//!
...
//!     action: hmanlab::providers::quota::QuotaAction::Reject,
//! };
//!
//! // Record some usage
...
/// `~/.hmanlab/quota/usage.json` after every `record()` call (best-effort;
...
impl QuotaStore {
    /// Load usage state from `~/.hmanlab/quota/usage.json`.
    ///
...
/// Canonical path for the usage file: `~/.hmanlab/quota/usage.json`.
fn dirs_path() -> PathBuf {
    let base = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    base.join(".hmanlab").join("quota").join("usage.json")
}
