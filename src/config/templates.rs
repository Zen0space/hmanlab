//! Agent template system for HmanLab
//!
...
/// loaded from `~/.hmanlab/templates/` can override built-in templates with
/// the same name.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn test_load_from_directory_with_json_files() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_load");
    ...
    fn test_load_from_empty_directory() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_empty");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_load_from_nonexistent_directory() {
        let path = Path::new("/tmp/hmanlab_tpl_nonexistent_98765");
    ...
    fn test_load_invalid_json_file() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_invalid");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_merge_from_dir() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_merge");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_load_from_dir_not_a_directory() {
        let temp_file = std::env::temp_dir().join("hmanlab_tpl_test_notdir.txt");
    ...
    fn test_load_toml_template_from_dir() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_toml");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
...
"#;
    ...
    fn test_load_mixed_json_and_toml_templates() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_mixed");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    fn test_invalid_toml_skipped() {
        let temp_dir = std::env::temp_dir().join("hmanlab_tpl_test_bad_toml");
        let _ = fs::remove_dir_all(&temp_dir);
        fs::create_dir_all(&temp_dir).unwrap();
