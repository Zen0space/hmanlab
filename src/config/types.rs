//! Configuration type definitions for HmanLab
//!
...
/// Main configuration struct for HmanLab
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
#[derive(Default)]
pub struct Config {
...
///
/// When enabled, HmanLab connects to an r8r instance over WebSocket to
...
/// Persists to `~/.hmanlab/cache/responses.json`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
...
/// Set `HMANLAB_DEFAULT_MODEL` at compile time to override.
const COMPILE_TIME_DEFAULT_MODEL: &str = match option_env!("HMANLAB_DEFAULT_MODEL") {
    Some(v) => v,
...
impl Default for AgentDefaults {
    fn default() -> Self {
        Self {
            workspace: "~/.hmanlab/workspace".to_string(),
...
pub struct ChannelsConfig {
    ...
    /// Directory for channel plugins (default: ~/.hmanlab/channels/)
    #[serde(default)]
...
///
/// **Gateway mode (`hmanlab gateway`):** `enabled` has no effect here.
/// The ACP stdio transport is exclusively for `hmanlab acp`, where the process
...
///
/// **`hmanlab acp`:** reads `allow_from`, `deny_by_default`, and `http` from
...
pub struct AcpChannelConfig {
    ...
    /// `hmanlab acp` always starts ACP stdio regardless of this flag.
    #[serde(default)]
    pub enabled: bool,
...
pub struct MqttChannelConfig {
    ...
    /// Topics to subscribe to for inbound messages (e.g., ["hmanlab/inbox/#"]).
    pub subscribe_topics: Vec<String>,
    /// Topic prefix for publishing responses (e.g., "hmanlab/outbox").
...
impl Default for MqttChannelConfig {
    fn default() -> Self {
        Self {
            ...
            client_id: "hmanlab-agent".to_string(),
            subscribe_topics: vec!["hmanlab/inbox/#".to_string()],
            publish_prefix: "hmanlab/outbox".to_string(),
            qos: 1,
            username: String::new(),
            password: String::new(),
            allow_from: Vec::new(),
...
fn default_webhook_signature_header() -> String {
    "X-HmanLab-Signature-256".to_string()
}
...
pub struct WhatsAppWebConfig {
    ...
    /// Default: ~/.hmanlab/state/whatsapp_web
...
fn default_whatsapp_web_auth_dir() -> String {
    "~/.hmanlab/state/whatsapp_web".to_string()
}
...
pub struct ToolsConfig {
    ...
    /// Enable when using HmanLab as a coding agent. The built-in "coder"
...
fn default_mount_allowlist_path() -> String {
    "~/.hmanlab/mount-allowlist.json".to_string()
}
...
impl Default for ContainerAgentConfig {
    fn default() -> Self {
        Self {
            ...
            image: "hmanlab:latest".to_string(),
            docker_binary: None,
...
///
/// ```
/// use hmanlab::config::CustomToolDef;
/// use std::collections::HashMap;
///
