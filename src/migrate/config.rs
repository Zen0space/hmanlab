//! OpenClaw config → HmanLab config conversion.
...
/// Convert an OpenClaw config (parsed JSON `Value`) into a HmanLab `Config`,
...
pub fn convert_config(openclaw: &Value, existing: &mut Config) -> MigrationConfigResult {
    ...
    // Slack — OpenClaw uses "token", HmanLab uses "bot_token"
...
fn check_not_portable(
    ...
    if pointer(oc, &["session", "scope"]).is_some() {
        not_portable
            .push("session.scope — HmanLab uses container-per-request isolation instead".into());
    }
    ...
    if pointer(oc, &["tools", "profile"]).is_some() {
        not_portable.push("tools.profile — use HmanLab's approval gate instead".into());
    }
    ...
    if let Some(provider) = str_at(oc, &["tools", "web", "search", "provider"]) {
        if provider != "brave" {
            skipped.push((
                ...
                "HmanLab supports Brave Search only".into(),
            ));
        }
    }
    ...
    if pointer(oc, &["auth", "profiles"]).is_some() {
        not_portable.push("auth.profiles — HmanLab uses API key authentication only".into());
    }
    ...
    if pointer(oc, &["plugins", "installs"]).is_some() {
        not_portable.push("plugins.installs — HmanLab uses a different plugin system".into());
    }
}
