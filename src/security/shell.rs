...
const LITERAL_BLOCKED_PATTERNS: &[&str] = &[
    ...
    ".aws/credentials",
    ".kube/config",
    // HmanLab's own config (contains API keys and channel tokens)
    ".hmanlab/config.json",
    ".hmanlab/config.yaml",
];
...
mod tests {
    ...
    #[test]
    fn test_hmanlab_config_blocked() {
        let config = ShellSecurityConfig::new();

        assert!(config
            .validate_command("cat ~/.hmanlab/config.json")
            .is_err());
        assert!(config
            .validate_command("cat ~/.hmanlab/config.yaml")
            .is_err());
        assert!(config
            .validate_command("cat /home/user/.hmanlab/config.json")
            .is_err());
        // Reading other hmanlab files (non-config) should be fine
        assert!(config
            .validate_command("cat ~/.hmanlab/skills/SKILL.md")
            .is_ok());
    }
