...
//!
//! Persists paired devices to `~/.hmanlab/security/paired_devices.json`.
//!
//! # Security notes
//!
...
mod tests {
    ...
    fn test_manager() -> PairingManager {
        ...
        PairingManager {
            ...
            path: PathBuf::from(format!("/tmp/hmanlab-test-pairing-{tid:?}-{id}.json")),
            pending_code: None,
            lockouts: HashMap::new(),
            max_attempts: 5,
    ...
    fn test_validate_token_no_disk_write() {
        ...
        let dir = std::env::temp_dir().join(format!("hmanlab-pairing-nodefer-{}", Uuid::new_v4()));
    ...
    fn test_store_persistence_roundtrip() {
        let dir = std::env::temp_dir().join(format!("hmanlab-pairing-test-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("paired_devices.json");
    ...
    fn test_load_from_disk_corrupt_file() {
        let dir = std::env::temp_dir().join(format!("hmanlab-pairing-corrupt-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("paired_devices.json");
    ...
    fn test_load_from_disk_missing_file() {
        let path = PathBuf::from("/tmp/hmanlab-pairing-nonexistent-12345.json");
        let mgr = PairingManager::with_path(path, 5, 300);
