...
///
/// Resolution order:
/// 1. `HMANLAB_MASTER_KEY` environment variable (hex-encoded 32 bytes)
...
pub fn resolve_master_key(interactive: bool) -> Result<SecretEncryption> {
    ...
    if let Ok(hex_key) = std::env::var("HMANLAB_MASTER_KEY") {
        let bytes = hex::decode(hex_key.trim())
            .map_err(|e| ZeptoError::Config(format!("HMANLAB_MASTER_KEY is not valid hex: {e}")))?;

        if bytes.len() != 32 {
            return Err(ZeptoError::Config(format!(
                "HMANLAB_MASTER_KEY must be 64 hex chars (32 bytes), got {} bytes",
                bytes.len()
            )));
        }
    ...
    Err(ZeptoError::Config(
        "no master key available: set HMANLAB_MASTER_KEY env var or use interactive mode".into(),
    ))
}
