...
//!
//! Persists to `~/.hmanlab/deps/registry.json`.
...
impl Registry {
    ...
    pub fn default_path() -> PathBuf {
        dirs::home_dir()
            ...
            .join(".hmanlab/deps/registry.json")
    }
}
...
mod tests {
    ...
    fn test_entry(name: &str) -> RegistryEntry {
        RegistryEntry {
            ...
            path: format!("~/.hmanlab/deps/bin/{}", name),
            running: false,
            pid: None,
        }
    }
    ...
    fn test_registry_save_and_load() {
        let dir = std::env::temp_dir().join("hmanlab_test_registry");
    ...
    fn test_registry_load_nonexistent() {
        let path = PathBuf::from("/tmp/nonexistent_hmanlab_registry.json");
    ...
    fn test_registry_load_empty_file() {
        let dir = std::env::temp_dir().join("hmanlab_test_registry_empty");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join("registry.json");
    ...
    fn test_registry_default_path() {
        ...
        assert!(path_str.contains(".hmanlab/deps/registry.json"));
    }
}
