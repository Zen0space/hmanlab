//! Bridge event types for r8r <-> HmanLab communication.
//!
...
pub enum BridgeEvent {
    ...
    // HmanLab -> r8r
    /// An approval decision from HmanLab.
    ApprovalDecision {
        approval_id: String,
        execution_id: String,
        node_id: String,
    ...
    /// A request from HmanLab to trigger a workflow.
    WorkflowTrigger {
        workflow: String,
        params: Value,
        triggered_by: String,
    ...
    /// A health ping from HmanLab (keepalive).
    HealthPing,
}
...
impl BridgeEvent {
    ...
    pub fn event_type_str(&self) -> &'static str {
        match self {
            ...
            BridgeEvent::ApprovalDecision { .. } => "hmanlab.approval.decision",
            BridgeEvent::WorkflowTrigger { .. } => "hmanlab.workflow.trigger",
            BridgeEvent::HealthPing => "hmanlab.health.ping",
        }
    }
    ...
    pub fn from_type_and_data(event_type: &str, data: &Value) -> Result<Self, String> {
        match event_type {
            ...
            "hmanlab.health.ping" => Ok(BridgeEvent::HealthPing),
