...
//!
//! Persists to `~/.hmanlab/cache/responses.json`. Cache key is a SHA-256
...
impl ResponseCache {
    ...
    ///
    /// Loads existing entries from `~/.hmanlab/cache/responses.json` on disk.
...
mod tests {
    ...
    fn test_cache() -> ResponseCache {
        ...
        ResponseCache {
            ...
            path: PathBuf::from(format!("/tmp/hmanlab-test-cache-{tid:?}-{id}.json")),
            ttl_secs: 3600,
            max_entries: 5,
        }
    }
    ...
    fn test_max_entries_zero_clamped() {
        let cache = ResponseCache {
            store: CacheStore::default(),
            path: PathBuf::from("/tmp/hmanlab-test-clamp.json"),
            ttl_secs: 3600,
            max_entries: 0,
        };
