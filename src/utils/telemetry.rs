...
/// - `hmanlab_tool_calls_total` (counter)
/// - `hmanlab_tool_errors_total` (counter)
/// - `hmanlab_tool_duration_seconds_sum` (counter)
/// - `hmanlab_tool_duration_seconds_min` (gauge)
/// - `hmanlab_tool_duration_seconds_max` (gauge)
/// - `hmanlab_tokens_input_total` (counter)
/// - `hmanlab_tokens_output_total` (counter)
/// - `hmanlab_session_duration_seconds` (gauge)
pub fn render_prometheus(collector: &MetricsCollector) -> String {
    ...
    // --- tool calls total ---
    out.push_str("# HELP hmanlab_tool_calls_total Total number of tool calls.\n");
    out.push_str("# TYPE hmanlab_tool_calls_total counter\n");
    for (name, m) in &tools {
        out.push_str(&format!(
            "hmanlab_tool_calls_total{{tool=\"{}\"}} {}\n",
            name, m.call_count,
        ));
    }

    // --- tool errors total ---
    out.push_str("# HELP hmanlab_tool_errors_total Total number of tool call errors.\n");
    out.push_str("# TYPE hmanlab_tool_errors_total counter\n");
    for (name, m) in &tools {
        out.push_str(&format!(
            "hmanlab_tool_errors_total{{tool=\"{}\"}} {}\n",
            name, m.error_count,
        ));
    }
    ...
    // --- tool duration sum ---
    out.push_str(
        "# HELP hmanlab_tool_duration_seconds_sum Cumulative tool call duration in seconds.\n",
    );
    out.push_str("# TYPE hmanlab_tool_duration_seconds_sum counter\n");
    for (name, m) in &tools {
        out.push_str(&format!(
            "hmanlab_tool_duration_seconds_sum{{tool=\"{}\"}} {:.6}\n",
            name,
    ...
    // --- tool duration min ---
    out.push_str(
        "# HELP hmanlab_tool_duration_seconds_min Minimum observed tool call duration in seconds.\n",
    );
    out.push_str("# TYPE hmanlab_tool_duration_seconds_min gauge\n");
    for (name, m) in &tools {
        let val = m.min_duration.map_or(0.0, |d| d.as_secs_f64());
        out.push_str(&format!(
            "hmanlab_tool_duration_seconds_min{{tool=\"{}\"}} {:.6}\n",
            name, val,
        ));
    }
    ...
    // --- tool duration max ---
    out.push_str(
        "# HELP hmanlab_tool_duration_seconds_max Maximum observed tool call duration in seconds.\n",
    );
    out.push_str("# TYPE hmanlab_tool_duration_seconds_max gauge\n");
    for (name, m) in &tools {
        let val = m.max_duration.map_or(0.0, |d| d.as_secs_f64());
        out.push_str(&format!(
            "hmanlab_tool_duration_seconds_max{{tool=\"{}\"}} {:.6}\n",
            name, val,
        ));
    }
    ...
    out.push_str("# HELP hmanlab_tokens_input_total Total input tokens consumed.\n");
    out.push_str("# TYPE hmanlab_tokens_input_total counter\n");
    out.push_str(&format!("hmanlab_tokens_input_total {}\n", tokens_in));

    out.push_str("# HELP hmanlab_tokens_output_total Total output tokens produced.\n");
    out.push_str("# TYPE hmanlab_tokens_output_total counter\n");
    out.push_str(&format!("hmanlab_tokens_output_total {}\n", tokens_out));

    // --- session duration ---
    out.push_str("# HELP hmanlab_session_duration_seconds Session uptime in seconds.\n");
    out.push_str("# TYPE hmanlab_session_duration_seconds gauge\n");
    out.push_str(&format!(
        "hmanlab_session_duration_seconds {:.6}\n",
...
mod tests {
    ...
    fn test_render_prometheus_empty_metrics() {
        ...
        assert!(output.contains("# HELP hmanlab_tokens_input_total"));
        assert!(output.contains("# TYPE hmanlab_tokens_input_total counter"));
        assert!(output.contains("hmanlab_tokens_input_total 0"));
        assert!(output.contains("hmanlab_tokens_output_total 0"));
        assert!(output.contains("# HELP hmanlab_session_duration_seconds"));
        assert!(output.contains("hmanlab_session_duration_seconds"));
    ...
    fn test_render_prometheus_populated_metrics() {
        ...
        // Tool call counts.
        assert!(output.contains("hmanlab_tool_calls_total{tool=\"shell\"} 2"));
        assert!(output.contains("hmanlab_tool_calls_total{tool=\"read_file\"} 1"));

        // Error counts.
        assert!(output.contains("hmanlab_tool_errors_total{tool=\"shell\"} 1"));
        assert!(output.contains("hmanlab_tool_errors_total{tool=\"read_file\"} 0"));

        // Tokens.
        assert!(output.contains("hmanlab_tokens_input_total 1500"));
        assert!(output.contains("hmanlab_tokens_output_total 800"));
    }
    ...
    fn test_render_prometheus_contains_expected_metric_families() {
        ...
        let expected_families = [
            "hmanlab_tool_calls_total",
            "hmanlab_tool_errors_total",
            "hmanlab_tool_duration_seconds_sum",
            "hmanlab_tool_duration_seconds_min",
            "hmanlab_tool_duration_seconds_max",
            "hmanlab_tokens_input_total",
            "hmanlab_tokens_output_total",
            "hmanlab_session_duration_seconds",
        ];
