...
use hmanlab::agent::{AgentLoop, ContextBuilder, RuntimeContext};
use hmanlab::bus::MessageBus;
use hmanlab::config::templates::{AgentTemplate, TemplateRegistry};
use hmanlab::config::{Config, MemoryBackend, MemoryCitationsMode};
use hmanlab::hands::resolve_hand;
use hmanlab::providers::{
    resolve_runtime_providers, FallbackProvider, LLMProvider, ProviderPlugin,
};
use hmanlab::session::SessionManager;
use hmanlab::skills::SkillsLoader;
use hmanlab::tools::approval::ApprovalPolicyConfig;
use hmanlab::tools::delegate::DelegateTool;
use hmanlab::tools::spawn::SpawnTool;
...
// Provider functions extracted to hmanlab::kernel::provider.
// Re-import for use within this module.
use hmanlab::kernel::provider::{apply_retry_wrapper, provider_from_runtime_selection};
...
pub(crate) async fn create_agent_with_template(
    ...
    if let Some(tpl) = &template {
        ...
        if let Some(budget) = tpl.max_token_budget {
            config.agents.defaults.token_budget = hmanlab::agent::budget::resolve_token_budget(
    ...
    let kernel = hmanlab::kernel::ZeptoKernel::boot(
        config.clone(),
        bus.clone(),
        template.as_ref(),
    ...
    let filter = hmanlab::kernel::ToolFilter::from_config(
        &config,
        template.as_ref(),
        active_hand.as_ref().map(|h| &h.manifest),
    );
    ...
    if filter.is_enabled("google") {
        ...
        if let Some(token) = google_token {
            agent
                .register_tool(Box::new(hmanlab::tools::GoogleTool::new(
                    &token,
    ...
    let unsupported = hmanlab::providers::configured_unsupported_provider_names(&config);
...
pub(crate) async fn fetch_provider_models(
    selection: &hmanlab::providers::RuntimeProviderSelection,
    ...
    let (url, is_ollama) = match provider {
        ...
        _ => {
            ...
            let registry_base = hmanlab::providers::PROVIDER_REGISTRY
                .iter()
...
async fn resolve_google_token(config: &Config) -> Option<String> {
    ...
    if let Ok(data) = tokio::fs::read_to_string(&token_path).await {
        if let Ok(token_set) = serde_json::from_str::<hmanlab::auth::OAuthTokenSet>(&data) {
...
pub(crate) fn model_provider_mismatch_warning(
    ...
    if let Some(inferred) = hmanlab::providers::provider_name_for_model(model) {
...
mod tests {
    ...
    // Provider chain tests moved to hmanlab::kernel::provider::tests
