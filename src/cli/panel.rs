//! `hmanlab panel` command — install, start, auth management.

use anyhow::{Context, Result};
use hmanlab::api::auth::generate_api_token;
use hmanlab::api::config::PanelConfig;
use hmanlab::api::events::EventBus;
use hmanlab::api::server::{start_server, AppState};
use hmanlab::config::Config;
use hmanlab::providers::openai::OpenAIProvider;
...
/// Main entry point for `hmanlab panel`.
...
/// 2. `~/.hmanlab/panel/dist/` — downloaded/installed assets
fn resolve_panel_dir() -> Option<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        let global = home.join(".hmanlab/panel/dist");
...
/// 2. `~/.hmanlab/panel/` — user-level installed source
fn resolve_panel_source_dir() -> Option<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        let global = home.join(".hmanlab/panel");
...
async fn cmd_start(
    ...
    let static_dir = if api_only || dev {
        ...
        match resolve_panel_dir() {
            ...
            None => {
                println!("Panel assets not found. Run 'hmanlab panel install' first.");
    ...
    let task_store = std::sync::Arc::new(hmanlab::api::tasks::TaskStore::new(task_store_path));
    ...
    if download {
        anyhow::bail!(
            "Downloading pre-built panel is not yet implemented. \
             Use `hmanlab panel install` (without --download) to build from source."
        );
    } else {
        println!("Installing HmanLab Panel...\n");
        ...
        let panel_dir = resolve_panel_source_dir().ok_or_else(|| {
            anyhow::anyhow!(
                "Panel source not found (no package.json in ./panel/ or \
                     ~/.hmanlab/panel/). Clone the repo or run from the \
                     HmanLab source directory."
            )
        })?;
    ...
    match action {
        PanelAuthAction::Status => {
            ...
            if tp.exists() {
                ...
                println!("No token file found. Run 'hmanlab panel install' to generate one.");
            }
            Ok(())
        }
        PanelAuthAction::Mode { mode } => {
            anyhow::bail!(
                "Setting auth mode to '{mode}' is not yet implemented. \
                 Edit ~/.hmanlab/config.json manually to set panel.auth_mode."
            );
        }
        PanelAuthAction::ResetPassword => {
            anyhow::bail!(
                "Password reset is not yet implemented. \
                 Edit ~/.hmanlab/config.json manually to set panel.password_hash."
            );
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_token_path_contains_hmanlab() {
        let path = token_path();
        let s = path.to_str().unwrap();
        assert!(
            s.contains(".hmanlab"),
            "token path must be inside .hmanlab dir"
        );
        assert!(
