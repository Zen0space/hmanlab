//! Doctor — system diagnostics for HmanLab.
...
fn check_workspace_writable(workspace: &Path, diags: &mut Vec<DiagItem>) {
    ...
    let probe = workspace.join(".hmanlab_doctor_probe");
...
pub(crate) async fn cmd_doctor(online: bool) -> Result<()> {
    let config = match Config::load() {
        ...
        Err(e) => {
            ...
            println!();
            println!("Run `hmanlab onboard` to create a configuration.");
            return Ok(());
        }
    };
    ...
    if errors > 0 {
        println!();
        println!("Fix the errors above to ensure HmanLab works correctly.");
    }
...
mod tests {
    ...
    fn test_check_binary_missing() {
        ...
        let mut diags = Vec::new();
        check_binary("hmanlab_nonexistent_a8f3e2d1b9c7", &mut diags);
        assert!(
