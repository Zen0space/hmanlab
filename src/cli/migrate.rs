//! `hmanlab migrate` — import config and skills from an OpenClaw installation.
...
use hmanlab::migrate::{self, MigrationReport};
...
pub(crate) async fn cmd_migrate(from: Option<String>, yes: bool, dry_run: bool) -> Result<()> {
    ...
    let do_skills = if skill_dirs.is_empty() || skill_count == 0 {
        ...
        print!("Copy {} skills to HmanLab? [Y/n]: ", skill_count);
        io::stdout().flush()?;
        let answer = read_line()?.to_ascii_lowercase();
        answer.is_empty() || answer == "y" || answer == "yes"
    };
    ...
    if !report.not_portable.is_empty() {
        ...
        println!("  https://hmanlab.pages.dev/docs/guides/migration/");
        println!();
    }
