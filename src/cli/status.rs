...
use hmanlab::auth;
use hmanlab::auth::codex_import;
use hmanlab::config::{Config, ContainerAgentBackend, ProviderConfig};
...
async fn cmd_auth_login(provider: Option<String>) -> Result<()> {
    ...
    let oauth_config = auth::provider_oauth_config(&provider).ok_or_else(|| {
        anyhow::anyhow!(
            "Provider '{}' does not support OAuth authentication.\n\
             ...
             export HMANLAB_PROVIDERS_{}_API_KEY=your-key-here",
            provider,
            auth::oauth_supported_providers().join(", "),
    ...
    println!("at any time. If blocked, HmanLab will fall back to your API key.");
    println!();

    let provider_env = format!(
        "HMANLAB_PROVIDERS_{}_OAUTH_CLIENT_ID",
        provider.to_uppercase()
    );
    let client_id = std::env::var(&provider_env)
        ...
        .or_else(|| {
            std::env::var("HMANLAB_OAUTH_CLIENT_ID")
                .ok()
                .filter(|v| !v.trim().is_empty())
        })
        .ok_or_else(|| {
            anyhow::anyhow!(
                "OAuth client_id is required for '{}'.\n\
                 ...
                 or:\n\
                   export HMANLAB_OAUTH_CLIENT_ID=your-client-id\n\n\
...
async fn cmd_auth_login_openai() -> Result<()> {
    ...
    println!("  export HMANLAB_PROVIDERS_OPENAI_API_KEY=your-key-here");
    println!();
...
fn cmd_auth_logout(provider: Option<String>) -> Result<()> {
    let encryption = match hmanlab::security::encryption::resolve_master_key(true) {
        Ok(enc) => enc,
        Err(_) => {
    ...
    if let Some(provider) = provider {
        ...
        match store.list() {
            ...
            Ok(entries) => {
                ...
                println!();
                println!("Specify a provider to log out: hmanlab auth logout <provider>");
            }
...
async fn cmd_auth_refresh(provider: &str) -> Result<()> {
    ...
    match auth::refresh::ensure_fresh_token(&store, provider).await {
        ...
        Err(e) => {
            ...
            println!();
            println!("Try logging in again: hmanlab auth login {}", provider);
        }
    }
...
pub(crate) async fn cmd_auth_setup_token() -> Result<()> {
    ...
    println!("If revoked, HmanLab will fall back to your API key.");
    println!();
    ...
    println!();
    println!("Test with: hmanlab agent -m \"Hello\"");
...
async fn cmd_auth_status() -> Result<()> {
    ...
    let token_store = match hmanlab::security::encryption::resolve_master_key(false) {
...
pub(crate) async fn cmd_status() -> Result<()> {
    ...
    if ltm_path.exists() {
        match hmanlab::memory::longterm::LongTermMemory::new() {
            Ok(mem) => {
                let count = mem.count();
