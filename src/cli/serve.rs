//! `hmanlab serve` — standalone OpenAI-compatible API server.
//!
...
pub async fn cmd_serve(port: u16, bind: String) -> Result<()> {
    ...
    let bus = Arc::new(hmanlab::bus::MessageBus::new());
    ...
    let event_bus = hmanlab::api::events::EventBus::new(4);
    let mut state = hmanlab::api::server::AppState::new(String::new(), event_bus);
    ...
    let app = Router::new()
        .route(
            "/v1/chat/completions",
            post(hmanlab::api::routes::openai::chat_completions),
        )
        .route("/v1/models", get(hmanlab::api::routes::openai::list_models))
        .with_state(shared);
