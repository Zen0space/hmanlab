...
async fn cmd_config_check() -> Result<()> {
    ...
    let diagnostics = hmanlab::config::validate::validate_config(&raw);
    ...
    let mut errors = diagnostics
        .iter()
        .filter(|d| d.level == hmanlab::config::validate::DiagnosticLevel::Error)
        .count();
    let mut warnings = diagnostics
        .iter()
        .filter(|d| d.level == hmanlab::config::validate::DiagnosticLevel::Warn)
        .count();

    // Validate custom tool definitions
    let config = Config::load().unwrap_or_default();
    let tool_warnings = hmanlab::config::validate::validate_custom_tools(&config);
