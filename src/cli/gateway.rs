...
use hmanlab::bus::MessageBus;
use hmanlab::channels::{register_configured_channels_with_approval, ChannelManager};
use hmanlab::config::watcher::ConfigWatcher;
use hmanlab::config::{Config, ContainerAgentBackend};
use hmanlab::health::{
    health_port, start_health_server, start_health_server_legacy, start_periodic_usage_flush,
    HealthRegistry, UsageMetrics,
};
use hmanlab::heartbeat::{ensure_heartbeat_file, HeartbeatService};
use hmanlab::providers::{
...
pub(crate) async fn cmd_gateway(
    ...
    println!("Starting HmanLab Gateway...");
    ...
    let guard = if config.gateway.startup_guard.enabled {
        let g = hmanlab::StartupGuard::new(
    ...
    let mut _tunnel: Option<Box<dyn hmanlab::tunnel::TunnelProvider>> = None;
    ...
    if let Some(ref provider) = tunnel_provider {
        ...
        let mut t = hmanlab::tunnel::create_tunnel(&tunnel_config)
    ...
    let proxy_handle = if containerized {
        ...
        let backend = hmanlab::gateway::resolve_backend(&config.container_agent)
            .await
        ...
        match backend {
            hmanlab::gateway::ResolvedBackend::Docker => {
                validate_docker_available(configured_docker_binary(&config.container_agent))
                    .await?;
            }
            #[cfg(target_os = "macos")]
            hmanlab::gateway::ResolvedBackend::Apple => {
        ...
        if backend == hmanlab::gateway::ResolvedBackend::Docker {
        ...
        let proxy_instance = Arc::new(hmanlab::gateway::ContainerAgentProxy::new(
            config.clone(),
            bus.clone(),
            backend,
        ));
    ...
    if !containerized {
        let model_diags = hmanlab::config::validate::validate_model_provider_compat(&config);
        for diag in &model_diags {
            match diag.level {
                hmanlab::config::validate::DiagnosticLevel::Error => {
                    error!("{}", diag);
                }
                hmanlab::config::validate::DiagnosticLevel::Warn => {
                    warn!("{}", diag);
                }
                hmanlab::config::validate::DiagnosticLevel::Ok => {
                    info!("{}", diag);
                }
            }
        }
        let has_errors = model_diags
            .iter()
            .any(|d| d.level == hmanlab::config::validate::DiagnosticLevel::Error);
        if has_errors {
            eprintln!();
            eprintln!("ERROR: Model-provider mismatch detected.");
            eprintln!(
                "  Fix your config ({:?}) or run 'hmanlab onboard'.",
                Config::path()
            );
            eprintln!("  Run 'hmanlab config check' for details.");
            eprintln!();
            std::process::exit(1);
        }
    }

    // Create shared approval store for channel-based tool approval.
    let approval_store = if config.approval.channel_timeout_secs > 0 && !containerized {
        let store = hmanlab::tools::approval_store::ApprovalRequestStore::new();
        info!(
            "Channel-based approval enabled (timeout={}s)",
            config.approval.channel_timeout_secs
        );
        Some(store)
    } else {
        None
    ...
    // Install channel-based approval handler on the agent loop.
    if let (Some(ref agent), Some(ref store)) = (&agent, &approval_store) {
        let handler_bus = bus.clone();
        let handler_store = store.clone();
        let timeout_secs = config.approval.channel_timeout_secs;
        let current_ctx: Arc<
            tokio::sync::RwLock<Option<hmanlab::tools::channel_approval::ChannelApprovalContext>>,
        > = Arc::new(tokio::sync::RwLock::new(None));

        let ctx_for_handler = current_ctx.clone();
        agent
            .set_approval_handler(move |request| {
                let bus = handler_bus.clone();
                let store = handler_store.clone();
                let timeout = timeout_secs;
                let ctx_ref = ctx_for_handler.clone();
                Box::pin(async move {
                    let ctx = ctx_ref.read().await.clone().unwrap_or(
                        hmanlab::tools::channel_approval::ChannelApprovalContext {
                    ...
                    hmanlab::tools::channel_approval::channel_approval_inner(
                        bus, store, ctx, request, timeout,
                    )
                    .await
                })
            })
        ...
        // Store the context setter so we can update it per inbound message.
        // We'll update it from the agent loop's process_inbound_message flow
        // by setting it when a message arrives.
        // For now, we use a simpler approach: the agent loop will call
        // set_current_approval_context before each message processing.
        agent.set_approval_context(current_ctx).await;
    }
    ...
    // Register channels via factory with approval store.
    let channel_count = register_configured_channels_with_approval(
        &channel_manager,
        bus.clone(),
        &config,
    ...
    let _hygiene_handle = match hmanlab::memory::longterm::LongTermMemory::new() {
        Ok(ltm) => {
            let ltm = Arc::new(tokio::sync::Mutex::new(ltm));
            Some(hmanlab::memory::hygiene::start_hygiene_scheduler(
                ltm,
    ...
    let _device_handle =
        hmanlab::devices::DeviceService::new(config.devices.enabled, config.devices.monitor_usb)
            .start()
            .map(|mut rx| {
                tokio::spawn(async move {
        ...
        tokio::select! {
            ...
            maybe_cfg = reload_rx.recv() => {
                ...
                if changed_sections.contains(&"channels") {
                    ...
                    let count = register_configured_channels_with_approval(&new_manager, bus.clone(), &config, approval_store.clone()).await;
                    if count == 0 {
...
async fn validate_docker_available(docker_binary: &str) -> Result<()> {
    if !hmanlab::gateway::is_docker_available_with_binary(docker_binary).await {
        return Err(anyhow::anyhow!(
...
fn configured_docker_binary(config: &hmanlab::config::ContainerAgentConfig) -> &str {
    config
        .docker_binary
        .as_deref()
        .map(str::trim)
...
async fn validate_apple_available() -> Result<()> {
    if !hmanlab::gateway::is_apple_container_available().await {
        return Err(anyhow::anyhow!(
