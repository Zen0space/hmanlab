//! CLI channel management commands (hmanlab channel list|setup|test).
...
fn setup_webhook(config: &mut Config) -> Result<()> {
    ...
    println!("  Run 'hmanlab gateway' to start listening.");
    Ok(())
}
...
async fn cmd_channel_test(channel_name: &str) -> Result<()> {
    ...
    match channel_name {
        ...
        "whatsapp_cloud" => match config.channels.whatsapp_cloud {
            ...
            _ => {
                anyhow::bail!("WhatsApp Cloud channel not configured. Run 'hmanlab channel setup whatsapp_cloud' first.");
            }
        },
        "telegram" => {
...
async fn test_whatsapp_web(config: &Config) -> Result<()> {
    ...
    match config.channels.whatsapp_web {
        Some(ref c) if c.enabled => {
            ...
            println!("  Run 'hmanlab gateway' to connect and pair.");
            Ok(())
        }
        Some(_) => {
            anyhow::bail!(
                "WhatsApp Web channel is not enabled. Run 'hmanlab channel setup whatsapp_web' first."
            );
        }
        None => {
            anyhow::bail!(
                "WhatsApp Web channel not configured. Run 'hmanlab channel setup whatsapp_web' first."
            );
        }
    }
}
