...
use hmanlab::skills::{EnvSpec, Skill, SkillsLoader};
...
#     required: true
metadata: {{"hmanlab":{{"emoji":"📚","requires":{{}}}}}}
---
...
async fn cmd_skills_search(config: &Config, query: &str, source: &str) -> Result<()> {
    ...
    if source == "all" || source == "github" {
        let topics = &["hmanlab-skill", "openclaw-skill"];
        match hmanlab::skills::github_source::search_github(
            &client,
            query,
            topics,
    ...
    for r in &all_results {
        let source_label = match r.source {
            hmanlab::skills::github_source::SkillSource::GitHub => "github",
            hmanlab::skills::github_source::SkillSource::ClawHub => "clawhub",
        };
        println!(
            "  {} ({}) [{}] score={:.2} stars={}",
...
const COMMUNITY_REPO: &str = "qhkm/hmanlab-skills";
...
async fn cmd_skills_install(name: &str, github: Option<&str>) -> Result<()> {
    ...
    let skills_dir = hmanlab::config::Config::dir().join("skills");
...
async fn install_from_multi_skill_repo(
    ...
    let tmp_dir =
        std::env::temp_dir().join(format!("hmanlab-skill-install-{}", std::process::id()));
...
"#
    ...
    fn test_normalize_github_repo() {
        ...
        assert_eq!(
            normalize_github_repo("https://github.com/qhkm/hmanlab-skills/obsidian-vault"),
            "qhkm/hmanlab-skills/obsidian-vault"
        );
        // Shorthand with skill path
        assert_eq!(
            normalize_github_repo("qhkm/hmanlab-skills/weather"),
            "qhkm/hmanlab-skills/weather"
        );
    }
    ...
    fn test_copy_dir_recursive() {
        let tmp = std::env::temp_dir().join("hmanlab-test-copy-dir");
    ...
    fn test_community_repo_constant() {
        assert_eq!(COMMUNITY_REPO, "qhkm/hmanlab-skills");
