...
use hmanlab::bus::{InboundMessage, MessageBus};
use hmanlab::channels::model_switch::ModelOverride;
use hmanlab::config::Config;
use hmanlab::gateway::ipc::UsageSnapshot;
use hmanlab::health::UsageMetrics;
use hmanlab::providers::{
    configured_provider_names, resolve_runtime_provider, RUNTIME_SUPPORTED_PROVIDERS,
};
use hmanlab::tools::approval::{ApprovalRequest, ApprovalResponse};
...
pub(crate) async fn cmd_agent(
    ...
    if config.agents.defaults.workspace == "~/.hmanlab/workspace" {
    ...
    tokio::spawn(async move {
        ...
        use hmanlab::agent::ToolFeedbackPhase;
    ...
    if resolve_runtime_provider(&config).is_none() {
        ...
        if configured.is_empty() {
            eprintln!(
                "Warning: No AI provider configured. Set HMANLAB_PROVIDERS_ANTHROPIC_API_KEY"
            );
    ...
    {
        let provider_names = hmanlab::providers::configured_provider_names(&config);
    ...
    if let Some(msg) = message {
        ...
        if streaming {
            use hmanlab::providers::StreamEvent;
        ...
        println!("HmanLab Interactive Agent");
        ...
        let mut rl = if interactive_cli {
            match Editor::new() {
                Ok(mut editor) => {
                    ...
                    let history_path =
                        dirs::home_dir().map(|h| h.join(".hmanlab/state/repl_history"));
            ...
            if let Some(cmd) = input.strip_prefix('/') {
                match cmd {
                    ...
                    _ if cmd == "model" || cmd.starts_with("model ") => {
                        use hmanlab::channels::model_switch::{
                            format_model_list, parse_model_command, ModelCommand,
                        };
                        use hmanlab::providers::configured_provider_models;
                        if let Some(mcmd) = parse_model_command(input) {
                            match mcmd {
                                ...
                                ModelCommand::Fetch => {
                                    ...
                                    let selections =
                                        hmanlab::providers::resolve_runtime_providers(&config);
                                    if selections.is_empty() {
                                        println!("No providers configured. Run 'hmanlab onboard' to set up.");
                                    } else {
                                        for s in &selections {
                    ...
                    _ if cmd == "template" || cmd.starts_with("template ") => {
                        use hmanlab::config::templates::TemplateRegistry;
                    ...
                    "history" => {
                        println!("Use 'hmanlab history list' for full history.");
                        println!("This session's messages are tracked automatically.");
                        continue;
                    }
                    "memory" => {
                        println!("Use 'hmanlab memory list' or 'hmanlab memory search <query>'.");
                        continue;
                    }
                    "clear" => {
...
pub(crate) async fn cmd_agent_stdin() -> Result<()> {
    ...
    let request: hmanlab::gateway::AgentRequest =
        serde_json::from_str(&input).map_err(|e| anyhow::anyhow!("Invalid request JSON: {}", e))?;

    if let Err(e) = request.validate() {
        let response = hmanlab::gateway::AgentResponse::error(
            &request.request_id,
            &e.to_string(),
            "INVALID_REQUEST",
        );
    ...
    let hmanlab::gateway::AgentRequest {
        request_id,
        message,
        agent_config,
        session,
    ...
    let response = match agent.process_message(&message).await {
        Ok(content) => {
            ...
            hmanlab::gateway::AgentResponse::success(&request_id, &content, updated_session)
                .with_usage(UsageSnapshot::from_metrics(&usage_metrics))
        }
        Err(e) => {
            hmanlab::gateway::AgentResponse::error(&request_id, &e.to_string(), "PROCESS_ERROR")
                .with_usage(UsageSnapshot::from_metrics(&usage_metrics))
        }
    };
...
fn format_cli_error(e: &dyn std::fmt::Display) -> String {
    ...
    if msg.contains("Authentication error") {
        format!(
            "{}\n\n  Fix: Check your API key. Run 'hmanlab auth status' to verify.\n  Or:  Set HMANLAB_PROVIDERS_ANTHROPIC_API_KEY=sk-ant-...",
            msg
        )
        ...
        format!(
            "{}\n\n  Fix: Check model name in config. Run 'hmanlab config check'.",
            msg
        )
        ...
        format!(
            "{}\n\n  Fix: Run 'hmanlab onboard' to set up an AI provider.",
            msg
        )
    } else {
        msg
...
mod tests {
    ...
    fn test_active_model_override_marks_current_in_formatted_list() {
        ...
        let list =
            hmanlab::channels::model_switch::format_model_list(&providers, current.as_ref(), &[]);
