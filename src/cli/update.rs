//! Self-update command.
//!
//! Downloads the latest HmanLab binary from GitHub Releases,
...
///
/// CI produces: `hmanlab-{os}-{arch}` where os = macos|linux,
/// arch = aarch64|x86_64.
fn platform_asset_name() -> Result<&'static str> {
    ...
    let name: &'static str = Box::leak(format!("hmanlab-{os}-{arch}").into_boxed_str());
    Ok(name)
}
...
async fn fetch_release(version: Option<&str>) -> Result<GitHubRelease> {
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", "hmanlab-self-update")
...
mod tests {
    ...
    fn test_platform_asset_name_valid() {
        ...
        assert!(name.starts_with("hmanlab-"));
