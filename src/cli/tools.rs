...
const TOOLS: &[ToolInfo] = &[
    ...
    ToolInfo {
        ...
        requires_config: true,
        config_hint: "Run `hmanlab auth login google` or set tools.google.access_token",
        opt_in: false,
    },
    ToolInfo {
        name: "r8r",
...
async fn cmd_tools_info(name: String) -> Result<()> {
    ...
    match TOOLS.iter().find(|t| t.name == name) {
        Some(t) => {
            ...
            if t.opt_in {
                ...
                if !coding_on {
                    ...
                    println!("       or set env var `HMANLAB_TOOLS_CODING_TOOLS=true`");
                }
            } else {
        ...
        None => {
            println!(
                "Unknown tool '{}'. Run 'hmanlab tools list' to see all tools.",
                name
            );
        }
    }
