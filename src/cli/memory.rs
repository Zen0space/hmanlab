...
use hmanlab::memory::longterm::LongTermMemory;
use hmanlab::memory::snapshot;
...
async fn cmd_memory_list(category: Option<String>) -> Result<()> {
    ...
    if entries.is_empty() {
        if let Some(cat) = category {
            ...
            println!("Store one: hmanlab memory set user:name \"Your Name\"");
        }
        return Ok(());
    }
...
async fn cmd_memory_stats() -> Result<()> {
    ...
    let path = hmanlab::config::Config::dir()
        .join("memory")
        .join("longterm.json");
    if path.exists() {
