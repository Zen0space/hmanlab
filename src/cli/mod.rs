...
enum Commands {
    ...
    Agent {
        ...
        /// Apply an agent template (built-in or ~/.hmanlab/templates/*.json)
        #[arg(long)]
    ...
    Uninstall {
        /// Also remove the current hmanlab binary for direct file installs
        #[arg(long)]
        remove_binary: bool,
...
pub enum MemoryAction {
    ...
    Export {
        /// Output file path (default: ~/.hmanlab/memory/snapshot.json)
        #[arg(long)]
        output: Option<std::path::PathBuf>,
    },
...
pub async fn run() -> Result<()> {
    ...
    let mut logging_cfg = hmanlab::config::Config::load()
        .map(|c| c.logging)
        .unwrap_or_default();
    ...
    //   RUST_LOG=debug hmanlab acp 2>acp.log
    ...
    hmanlab::utils::logging::init_logging(&logging_cfg);
    ...
    match cli.command {
        ...
        Some(Commands::Panel {
            ...
            #[cfg(feature = "panel")]
            {
                let config = hmanlab::config::Config::load()
...
/// Compatible with `acpx --agent 'hmanlab acp'` and any other ACP client.
async fn cmd_acp() -> Result<()> {
    use std::sync::Arc;

    use hmanlab::bus::MessageBus;
    use hmanlab::channels::Channel;
    use hmanlab::{AcpChannel, BaseChannelConfig, Config};
    ...
    match action {
        ...
        HardwareAction::Info { device } => match mgr.device_info(&device) {
            ...
            None => {
                ...
                println!();
                println!("Try: hmanlab hardware list");
...
fn cmd_panel_unavailable(
    ...
    let requested = match action {
        Some(PanelDisabledAction::Install { .. }) => "hmanlab panel install",
        Some(PanelDisabledAction::Auth { .. }) => "hmanlab panel auth",
        Some(PanelDisabledAction::Uninstall) => "hmanlab panel uninstall",
        None => "hmanlab panel",
    };

    anyhow::bail!(
        "This hmanlab binary was built without the optional 'panel' feature, so `{requested}` is unavailable.\n\
         ...
         Install with cargo using panel support:\n\
           cargo install hmanlab --git https://github.com/qhkm/hmanlab --features panel"
    );
}
...
async fn cmd_mcp_server(http: Option<String>) -> Result<()> {
    ...
    use hmanlab::mcp_server::McpServer;
    ...
    let kernel = hmanlab::kernel::ZeptoKernel::boot(config, bus, None, None).await?;
            ...
            #[cfg(not(feature = "panel"))]
            {
                ...
                anyhow::bail!(
                    "HTTP transport requires the 'panel' feature. \
                     ...
                     Or use stdio transport (default): hmanlab mcp-server"
                );
            }
        }
        None => {
...
mod tests {
    ...
    fn panel_subcommand_is_recognized_without_panel_feature() {
        let cli =
            Cli::try_parse_from(["hmanlab", "panel", "install"]).expect("parse panel install");
