...
/// agent.start().await?;
/// ```
type ApprovalContextRef = Arc<
    RwLock<Option<Arc<RwLock<Option<crate::tools::channel_approval::ChannelApprovalContext>>>>>,
>;
...
pub struct AgentLoop {
    ...
    /// Shared context for channel-based approval: stores current inbound message
    /// channel/chat info so the approval handler can route prompts correctly.
...
impl AgentLoop {
    ...
    pub fn new(config: Config, session_manager: SessionManager, bus: Arc<MessageBus>) -> Self {
        ...
        Self {
            ...
            approval_context: Arc::new(RwLock::new(None)),
            agent_mode,
            safety_layer,
            context_monitor,
    ...
    /// Set the shared approval context reference. The gateway sets this so
    /// the approval handler can read channel/chat metadata from the current
    /// inbound message.
    pub async fn set_approval_context(
        &self,
        ctx: Arc<RwLock<Option<crate::tools::channel_approval::ChannelApprovalContext>>>,
    ) {
        let mut slot = self.approval_context.write().await;
        *slot = Some(ctx);
    }
    ...
    async fn process_inbound_message(
        ...
        // Set the approval context so the channel approval handler knows
        // which channel/chat the current request came from.
        {
            let ctx_slot = self.approval_context.read().await;
            if let Some(ctx_ref) = ctx_slot.as_ref() {
                let mut ctx = ctx_ref.write().await;
                *ctx = Some(crate::tools::channel_approval::ChannelApprovalContext {
                    channel: msg.channel.clone(),
                    chat_id: msg.chat_id.clone(),
                    thread_id: msg.metadata.get("telegram_thread_id").cloned(),
                    reply_to: msg.metadata.get("telegram_message_id").cloned(),
                });
            }
        }
