//! High-level library facade for embedding HmanLab as a crate.
//!
//! `HmanLabAgent` provides a simple `chat()` method with persistent conversation
...
//!
//! ```rust,ignore
//! use hmanlab::agent::HmanLabAgent;
//! use hmanlab::{ClaudeProvider, EchoTool};
//!
//! let agent = HmanLabAgent::builder()
...
impl HmanLabAgent {
    ...
    pub fn builder() -> HmanLabAgentBuilder {
        HmanLabAgentBuilder::new()
    }
        ...
        F: Fn(&str, &str),
    {
        ...
        for iteration in 0..self.max_iterations {
            info!(
                "[HmanLabAgent] Iteration {}/{} — sending {} messages to LLM",
                iteration + 1,
                self.max_iterations,
                messages.len()
            );
            ...
            if !response.has_tool_calls() {
                ...
                info!(
                    "[HmanLabAgent] LLM returned text response: {:?}",
            ...
            info!(
                "[HmanLabAgent] LLM returned {} tool call(s), executing first: '{}'",
            ...
            info!(
                "[HmanLabAgent] Executing tool '{}' with args: {}",
                tc.name, args
            );
            ...
            let result = if self.tools.has(&tc.name) {
                ...
                .await
                {
                    warn!("[HmanLabAgent] Tool '{}' blocked: {}", tc.name, denial);
                    denial
                } else {
                    ...
                    match tokio::time::timeout(self.tool_timeout, execution).await {
                        Ok(Ok(Ok(output))) => {
                            ...
                            debug!(
                                "[HmanLabAgent] Tool '{}' succeeded: {}",
                                tc.name, output_preview
                            );
                            output.for_llm
                        }
                        Ok(Ok(Err(e))) => {
                            warn!("[HmanLabAgent] Tool '{}' failed: {}", tc.name, e);
                            format!("Tool error: {e}")
                        }
                        Ok(Err(_panic)) => {
                            warn!(
                                "[HmanLabAgent] Tool '{}' panicked during execution",
                                tc.name
                            );
                        ...
                        Err(_) => {
                            warn!(
                                "[HmanLabAgent] Tool '{}' timed out after {:?}",
                ...
                warn!("[HmanLabAgent] Unknown tool: {}", tc.name);
    ...
    pub async fn repair_history(&self) {
        ...
        while let Some(last) = history.last() {
            ...
            if matches!(last.role, Role::Assistant) && has_tool_calls {
                info!("[HmanLabAgent] Removing dangling assistant tool_call from history");
                history.pop();
            } else {
                break;
            }
...
/// Builder for `HmanLabAgent`.
pub struct HmanLabAgentBuilder {
...
impl HmanLabAgentBuilder {
    ...
    /// Build the `HmanLabAgent`.
    ///
    /// Returns `Err` if no provider was set.
    pub fn build(self) -> Result<HmanLabAgent> {
        let provider = self.provider.ok_or_else(|| {
            ZeptoError::Config(
                "HmanLabAgent requires a provider. Call .provider() on the builder.".into(),
            )
        })?;
...
impl Default for HmanLabAgentBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    async fn test_builder_no_provider() {
        let result = HmanLabAgent::builder().build();
