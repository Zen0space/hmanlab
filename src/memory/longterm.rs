//! Long-term memory store for HmanLab.
//!
...
//! single JSON file at `~/.hmanlab/memory/longterm.json`.
...
impl LongTermMemory {
    ...
    /// (`~/.hmanlab/memory/longterm.json`). Creates the file and parent
...
mod tests {
    ...
    async fn test_search_by_key() {
        ...
        mem.set("user:name", "Alice", "user", vec![], 1.0)
            .await
            .unwrap();
        mem.set("project:name", "HmanLab", "project", vec![], 1.0)
            .await
            .unwrap();
