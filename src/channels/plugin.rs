//! Channel Plugin Adapter for HmanLab
//!
...
/// channel plugins directory (`~/.hmanlab/channels/` by default).
...
/// # Arguments
///
/// * `plugin_dir` - The directory to scan (e.g., `~/.hmanlab/channels/`)
///
/// # Returns
///
...
/// Returns the default channel plugins directory (`~/.hmanlab/channels/`).
pub fn default_channel_plugins_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".hmanlab").join("channels"))
}
...
mod tests {
    ...
    fn test_default_channel_plugins_dir() {
        ...
        if let Some(d) = dir {
            ...
            assert!(d.to_string_lossy().contains(".hmanlab"));
        }
    }
