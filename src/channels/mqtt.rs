...
//!
//! ```text
//! hmanlab/inbox/{device_id}    # Device → Agent (inbound messages)
//! hmanlab/outbox/{device_id}   # Agent → Device (responses)
//! ```
//!
//! # Protocol
//!
...
mod inner {
    ...
    ///
    /// Given `"hmanlab/inbox/node-17"`, returns `Some("node-17")`.
    ...
    mod tests {
        ...
        fn test_extract_device_id() {
            assert_eq!(extract_device_id("hmanlab/inbox/node-17"), Some("node-17"));
            assert_eq!(extract_device_id("hmanlab/inbox/esp32-0"), Some("esp32-0"));
        ...
        fn test_mqtt_config_defaults() {
            ...
            assert_eq!(config.client_id, "hmanlab-agent");
            assert_eq!(config.subscribe_topics, vec!["hmanlab/inbox/#"]);
            assert_eq!(config.publish_prefix, "hmanlab/outbox");
