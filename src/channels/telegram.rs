...
//!
//! This module provides a Telegram bot channel for HmanLab using the teloxide library.
...
//! use hmanlab::config::TelegramConfig;
//! use hmanlab::channels::TelegramChannel;
//!
...
//! let channel = TelegramChannel::new(config, bus, "default-model".to_string(), vec![], vec![], false, None);
//! ```
...
use crate::tools::channel_approval;
...
struct OverridesDep {
    ...
    approval_store: Option<ApprovalRequestStore>,
}
...
pub struct TelegramChannel {
    ...
    http_client: reqwest::Client,
    /// Shared store for resolving channel-based tool approvals.
    approval_store: Option<ApprovalRequestStore>,
}
                        ...
                        |bot: Bot,
                         ...
                         http_client: reqwest::Client| async move {
                                ...
                                .or(if has_image { Some(BARE_PHOTO_PLACEHOLDER) } else { None })
                            {
                                ...
                                // Intercept /approve commands for channel-based tool approval
                                if let Some((approval_id, decision)) = channel_approval::parse_approval_text(text) {
                                    if let Some(ref store) = approval_store {
                                        let response: ApprovalResponse = decision.into();
                                        let resolved = store.resolve(&approval_id, response.clone()).await;
                                        let reply = if resolved {
                                            match response {
                                                ApprovalResponse::Approved => "\u{2705} Tool execution approved.".to_string(),
                                                ApprovalResponse::Denied(_) => "\u{274c} Tool execution denied.".to_string(),
                                                ApprovalResponse::TimedOut => "\u{23f0} Approval already timed out.".to_string(),
                                            }
                                        } else {
                                            format!("\u{26a0}\u{fe0f} Approval {} not found (may have expired).", approval_id)
                                        };
                    ...
                    Update::filter_callback_query().endpoint(
                        |bot: Bot,
                         ...
                         AllowUsernames(allow_usernames): AllowUsernames| async move {
                            ...
                            if !allowed {
                                let _ = bot.answer_callback_query(q.id).await;
                                if let Some(regular_msg) = q.message.as_ref().and_then(|m| m.regular_message()) {
                                    let _ = bot.send_message(
                                        regular_msg.chat.id,
                                        "\u{26d4} You are not authorized to approve tool executions.",
                                    ).await;
                                }
                                return Ok(());
                            }

                            let response = match decision_str {
                                "allow" => ApprovalResponse::Approved,
                                "deny" => ApprovalResponse::Denied("denied by user".to_string()),
                                _ => {
                                    let _ = bot.answer_callback_query(q.id).await;
                                    return Ok(());
                                }
                            ...
                            let resolved = if let Some(ref store) = approval_store {
                                store.resolve(approval_id, response.clone()).await
                            } else {
                                false
                            };
                            ...
                            if let Some(maybe_msg) = q.message.as_ref() {
                                let answer_text = if resolved {
                                    match &response {
                                        ApprovalResponse::Approved => "\u{2705} Approved".to_string(),
                                        ApprovalResponse::Denied(_) => "\u{274c} Denied".to_string(),
                                        ApprovalResponse::TimedOut => "\u{23f0} Already timed out".to_string(),
                                    }
                                } else {
                                    "\u{26a0}\u{fe0f} Not found (expired?)".to_string()
                                };
                                ...
                                let who = if username.is_empty() { user_id.clone() } else { format!("@{}", username) };
                                let new_text = if resolved {
                                    match &response {
                                        ApprovalResponse::Approved => {
                                            format!("\u{2705} Approved by {}", who)
                                        }
                                        ApprovalResponse::Denied(_) => {
                                            format!("\u{274c} Denied by {}", who)
                                        }
                                        ApprovalResponse::TimedOut => "Timed out".to_string(),
                                    }
                                } else {
                                    "Expired".to_string()
                                };
    ...
    async fn send(&self, msg: OutboundMessage) -> Result<()> {
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            if i == 0 {
                ...
                // Attach inline keyboard for approval prompts (last chunk = only chunk in most cases).
                if is_approval_prompt && i == chunks.len() - 1 {
                    if let Some(approval_id) = msg.metadata.get("approval_id") {
                        use teloxide::types::{InlineKeyboardButton, InlineKeyboardMarkup};

                        let kb = InlineKeyboardMarkup::new(vec![vec![
                            InlineKeyboardButton::callback(
                                "\u{2705} Allow".to_string(),
                                format!("/approve {} allow", approval_id),
                            ),
                            InlineKeyboardButton::callback(
