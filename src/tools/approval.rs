//! Tool approval system for HmanLab
//!
...
//!
//! ```rust
//! use hmanlab::tools::approval::{ApprovalConfig, ApprovalGate};
//!
...
/// - `channel_timeout_secs`: `300` (5 minutes, for channel-based approval)
...
pub struct ApprovalConfig {
    ...
    /// Timeout in seconds for channel-based approval prompts (e.g. Telegram
    /// inline keyboard buttons). If no response arrives within this time,
    /// the tool is denied. Defaults to 300 (5 minutes). `0` disables
    /// channel-based approval (falls back to the default no-handler behavior).
...
///
/// ```rust
/// use hmanlab::tools::approval::{ApprovalConfig, ApprovalPolicyConfig, ApprovalGate};
///
