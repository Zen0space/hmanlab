//! HTTP health server for HmanLab.
//!
...
/// # Example
/// ```
/// use hmanlab::health::{HealthRegistry, HealthCheck, HealthStatus};
...
pub async fn start_health_server_legacy(
    ...
    let host = std::env::var("HMANLAB_HEALTH_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
...
/// Resolve the health server port from `HMANLAB_HEALTH_PORT` env var,
/// falling back to the compiled-in default (9090).
pub fn health_port() -> u16 {
    std::env::var("HMANLAB_HEALTH_PORT")
        .ok()
...
mod tests {
    ...
    fn test_health_port_default() {
        std::env::remove_var("HMANLAB_HEALTH_PORT");
