//! HmanLab Load Testing Benchmark
//!
...
use hmanlab::providers::{ChatOptions, LLMProvider, LLMResponse, ToolDefinition};
use hmanlab::session::{Message, SessionManager};
use hmanlab::tools::EchoTool;

#[derive(Parser)]
#[command(name = "hmanlab-benchmark")]
#[command(about = "Load testing benchmark for HmanLab")]
struct Args {
...
impl LLMProvider for MockProvider {
    ...
    ) -> std::result::Result<LLMResponse, hmanlab::error::ZeptoError> {
        // Find the last user message and echo it back
        let response_text = messages
            .iter()
            .rev()
            .find(|m| matches!(m.role, hmanlab::session::Role::User))
