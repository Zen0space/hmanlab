...
async fn check_skill_md_exists(
    ...
    let mut request = client.get(&url).header("User-Agent", "hmanlab");
...
mod tests {
    ...
    fn test_build_search_url() {
        let url = build_search_url("web scraping", &["hmanlab-skill"]);
        assert!(url.contains("web+scraping"));
        assert!(url.contains("topic:hmanlab-skill"));
    }

    #[test]
    fn test_build_search_url_multiple_topics() {
        let url = build_search_url("test", &["hmanlab-skill", "openclaw-skill"]);
        assert!(url.contains("topic:hmanlab-skill"));
