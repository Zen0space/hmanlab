...
impl SkillsLoader {
    ...
    fn get_zeptometa(&self, skill: &Skill) -> ZeptoMetadata {
        skill
            ...
            .as_ref()
            .and_then(|value| {
                // Priority: hmanlab > clawdbot > openclaw > clawdis > raw object
                if let Some(scoped) = value.get("hmanlab") {
...
metadata: {"hmanlab":{"emoji":"🌤️","requires":{"bins":["curl"]}}}
---
# Weather
...
"#;
    ...
    #[test]
    fn test_hmanlab_metadata_takes_priority_over_openclaw() {
        let temp = tempfile::tempdir().unwrap();
        let ws = temp.path().join("skills");
        std::fs::create_dir_all(ws.join("dual")).unwrap();
        std::fs::write(
            ws.join("dual/SKILL.md"),
            "---\nname: dual\ndescription: Both namespaces\nmetadata: {\"hmanlab\":{\"emoji\":\"🦀\"},\"openclaw\":{\"emoji\":\"🦞\"}}\n---\nBody.",
        )
        .unwrap();
    ...
    fn test_openclaw_unknown_fields_ignored() {
        ...
        // Include OpenClaw-only fields that HmanLab doesn't have
        std::fs::write(
    ...
    fn test_any_bins_none_found() {
        ...
        std::fs::write(
            ...
            "---\nname: missing\ndescription: Missing bins\nmetadata: {\"hmanlab\":{\"requires\":{\"any_bins\":[\"zzz_nonexistent_1\",\"zzz_nonexistent_2\"]}}}\n---\nBody.",
        )
        .unwrap();
    ...
    #[test]
    fn test_hmanlab_priority_over_clawdbot() {
        let temp = tempfile::tempdir().unwrap();
        let ws = temp.path().join("skills");
        std::fs::create_dir_all(ws.join("prio")).unwrap();
        std::fs::write(
            ws.join("prio/SKILL.md"),
            "---\nname: prio\ndescription: Priority test\nmetadata: {\"hmanlab\":{\"emoji\":\"🦀\"},\"clawdbot\":{\"emoji\":\"🤖\"}}\n---\nBody.",
        )
        .unwrap();
