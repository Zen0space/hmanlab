...
### Added
- **ZeptoAgent library facade** — `ZeptoAgent::builder().provider(p).tool(t).build()` for embedding HmanLab as a crate in Tauri/Electron/GUI apps; persistent conversation history behind `Mutex` for thread-safe concurrent access (#154)
