...
HmanLab ships with 32 built-in tools. Most tools are available by default; `grep` and `find` are opt-in coding tools (see below).
...
Stored at `~/.hmanlab/memory/longterm.json`. Persists across sessions with access tracking.
