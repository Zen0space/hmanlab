---
title: Agent Loop
description: How HmanLab processes messages and executes tools
---

The agent loop is the core of HmanLab. It receives a message, builds context, calls an LLM, executes tool calls, and returns the response.
...
```bash
hmanlab agent --stream -m "Explain monads"
```
...
Configure via:
```bash
export HMANLAB_AGENTS_DEFAULTS_AGENT_TIMEOUT_SECS=600
```
