---
title: Tools
description: How agent tools work in HmanLab
---
...
HmanLab ships with 32 built-in tools:
...
- Using the `coder` template: `hmanlab agent --template coder -m "..."`
- Setting `tools.coding_tools: true` in `~/.hmanlab/config.json`
- Setting `HMANLAB_TOOLS_CODING_TOOLS=true` env var

## Parallel execution

When the LLM returns multiple tool calls in one response, HmanLab executes them concurrently using `futures::future::join_all`. This reduces latency when tools are independent.
...
You can add custom tools without modifying HmanLab's source code using the [plugin system](/docs/guides/plugins/). Plugins are JSON manifests that define tool name, parameters, and a command template.
