...
Providers are the LLM backends that power your agent. HmanLab supports 9 providers out of the box, all configurable via `~/.hmanlab/config.json` or environment variables.
...
The onboard wizard supports Anthropic, OpenAI, and OpenRouter. For other providers, edit `~/.hmanlab/config.json` directly or use environment variables.
...
```bash
export HMANLAB_PROVIDERS_OPENROUTER_API_KEY=sk-or-...
```
...
```bash
export HMANLAB_PROVIDERS_OLLAMA_API_KEY=ollama
```
...
```bash
export HMANLAB_PROVIDERS_GEMINI_API_KEY=AIza...
```
...
```bash
export HMANLAB_PROVIDERS_NVIDIA_API_KEY=nvapi-...
```
...
```bash
export HMANLAB_PROVIDERS_OPENAI_API_BASE=https://my-proxy.example.com/v1
```

## Provider stack

HmanLab wraps providers in a composable stack:
...
```bash
hmanlab agent --stream -m "Tell me a story"
```
...
HMANLAB_PROVIDERS_OPENROUTER_API_KEY=...
HMANLAB_PROVIDERS_GROQ_API_KEY=...
HMANLAB_PROVIDERS_OLLAMA_API_KEY=...
HMANLAB_PROVIDERS_VLLM_API_KEY=...
HMANLAB_PROVIDERS_GEMINI_API_KEY=...
HMANLAB_PROVIDERS_NVIDIA_API_KEY=...
HMANLAB_PROVIDERS_ZHIPU_API_KEY=...

# Custom base URLs
HMANLAB_PROVIDERS_OPENAI_API_BASE=...
HMANLAB_PROVIDERS_OLLAMA_API_BASE=...

# Retry
HMANLAB_PROVIDERS_RETRY_ENABLED=true
HMANLAB_PROVIDERS_RETRY_MAX_RETRIES=3
HMANLAB_PROVIDERS_RETRY_BASE_DELAY_MS=1000
HMANLAB_PROVIDERS_RETRY_MAX_DELAY_MS=30000

# Fallback
HMANLAB_PROVIDERS_FALLBACK_ENABLED=true
HMANLAB_PROVIDERS_FALLBACK_PROVIDER=openai
```
