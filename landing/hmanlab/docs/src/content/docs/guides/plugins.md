---
title: Plugins
description: Extend HmanLab with custom tools using JSON manifest plugins
---

The plugin system lets you add custom tools to your agent without modifying HmanLab's source code. Plugins are JSON manifest files that define a tool's name, parameters, and a command template.

## Plugin structure

Create a JSON file in `~/.hmanlab/plugins/`:
...
1. **Discovery** — At startup, HmanLab scans configured plugin directories for JSON files
...
```json
{
  "plugins": {
    "enabled": true,
    "directories": ["~/.hmanlab/plugins", "/opt/hmanlab/plugins"]
  }
}
```
