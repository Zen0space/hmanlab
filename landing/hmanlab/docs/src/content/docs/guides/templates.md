...
HmanLab ships with 4 built-in templates:
...
```bash
# Use a template
hmanlab agent --template researcher -m "Research Rust async patterns"

# List available templates
hmanlab template list

# Show template details
hmanlab template show coder
```

### Batch mode

```bash
hmanlab batch --input prompts.txt --template coder
```
...
Create a JSON file in `~/.hmanlab/templates/`:
...
```bash
hmanlab agent --template devops -m "Set up a GitHub Actions pipeline"
```
