---
title: Security
description: Security features and best practices for HmanLab
---

HmanLab is designed with security as a core concern. This guide covers the built-in security features and best practices for production deployments.
...
hmanlab gateway --containerized apple
```
...
HmanLab includes a multi-layered prompt injection detector that runs on all LLM inputs:
...
```bash
export HMANLAB_SAFETY_ENABLED=true  # default
```
...
```bash
export HMANLAB_SAFETY_LEAK_DETECTION_ENABLED=true  # default
```
...
- `HMANLAB_MASTER_KEY` environment variable (hex-encoded 32 bytes)
- A key file
- Interactive prompt
...
1. **Always use container isolation in production** — Run `hmanlab gateway --containerized`
...
9. **Encrypt secrets at rest** — Use `hmanlab secrets encrypt` to protect API keys in config
