...
use hmanlab::r8r_bridge::{Ack, BridgeEvent, BridgeEventEnvelope, R8rBridge};
        ...
        while let Some(Ok(msg)) = ws_read.next().await {
            if let WsMessage::Text(text) = msg {
                ...
                if let Ok(parsed) = serde_json::from_str::<Value>(&text_str) {
                    if parsed.get("type").and_then(|t| t.as_str()) == Some("hmanlab.health.ping") {
