...
/// Helper: run hmanlab with given args and return (exit_code, stdout, stderr).
fn run_cli(args: &[&str]) -> (i32, String, String) {
    let bin = env!("CARGO_BIN_EXE_hmanlab");
    let output = Command::new(bin)
        ...
        .output()
        .expect("failed to execute hmanlab binary");
...
fn cli_no_args_shows_help() {
    ...
    assert!(stdout.contains("hmanlab"));
}
