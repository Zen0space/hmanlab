...
//! Two test layers:
//!
//! 1. **Raw wire tests** — spawn `hmanlab acp` as a subprocess, drive it
...
//!    Gated behind `HMANLAB_E2E_LIVE` (requires a configured LLM provider).
//!
//! Run with:
//!
//! ```bash
//! cargo nextest run --test acp_acpx
//! HMANLAB_E2E_LIVE=1 cargo nextest run --test acp_acpx
//! ```
...
/// Path to the compiled hmanlab binary.
fn bin() -> &'static str {
    env!("CARGO_BIN_EXE_hmanlab")
}
...
/// A raw JSON-RPC connection to `hmanlab acp` over stdin/stdout.
struct AcpConn {
    child: Child,
    stdin: ChildStdin,
...
impl AcpConn {
    /// Spawn `hmanlab acp` and return a connected handle.
    async fn spawn() -> Self {
        let mut child = Command::new(bin())
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("failed to spawn hmanlab acp");
...
async fn test_initialize_agent_info_fields_are_strings() {
    ...
    assert_eq!(info["name"].as_str().unwrap(), "hmanlab");
    conn.shutdown().await;
}
...
/// Check whether the HMANLAB_E2E_LIVE gate is set.
fn e2e_live() -> bool {
    std::env::var("HMANLAB_E2E_LIVE").is_ok()
}

/// Run `acpx --agent 'hmanlab acp' --format json --approve-all exec <prompt>`
...
fn test_acpx_exec_basic_prompt() {
    if !e2e_live() {
        eprintln!("Skipping: set HMANLAB_E2E_LIVE=1 to run");
        return;
    }
...
fn test_acpx_session_lifecycle() {
    ...
    let run_acpx = |args: &[&str]| -> (bool, String, String) {
        let out = std::process::Command::new(&acpx)
            .args(args)
            .env("RUST_LOG", "")
            .env("HMANLAB_MASTER_KEY", master_key)
            .env("PATH", acpx_path_env(&acpx))
            .output()
