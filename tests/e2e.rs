...
//!   `HMANLAB_E2E_LIVE` environment variable.
//! - Tests requiring a running Docker daemon are gated behind the
//!   `HMANLAB_E2E_DOCKER` environment variable.
//!
...
use hmanlab::providers::{ChatOptions, LLMProvider, LLMResponse, LLMToolCall, ToolDefinition};
use hmanlab::session::{Message, SessionManager};
use hmanlab::tools::{EchoTool, ToolContext, ToolRegistry};
...
impl LLMProvider for MockBudgetExhaustingProvider {
    ...
    async fn chat(
        ...
        Ok(LLMResponse {
            ...
            tool_calls: vec![],
            usage: Some(hmanlab::providers::Usage {
...
fn is_live_enabled() -> bool {
    std::env::var("HMANLAB_E2E_LIVE").is_ok()
}

fn is_docker_enabled() -> bool {
    std::env::var("HMANLAB_E2E_DOCKER").is_ok()
}
...
async fn test_agent_start_and_respond() {
    ...
    let agent = hmanlab::agent::AgentLoop::new(config, session_manager, bus.clone());
...
async fn test_container_backend_resolution_docker() {
    let mut config = Config::default();
    config.container_agent.backend = hmanlab::config::ContainerAgentBackend::Docker;
...
fn test_agent_request_validation_rejects_mismatch() {
    let request = AgentRequest {
        ...
        agent_config: Config::default().agents.defaults,
        session: Some(hmanlab::session::Session::new("test:chat-b")),
    };
...
/// `HMANLAB_E2E_DOCKER` is set.
#[tokio::test]
async fn test_docker_availability() {
    if !is_docker_enabled() {
        eprintln!("Skipping Docker availability test (HMANLAB_E2E_DOCKER not set)");
        return;
    }

    let available = hmanlab::gateway::is_docker_available().await;
    assert!(
        available,
        "Docker should be available when HMANLAB_E2E_DOCKER is set"
    );
}
...
/// `HMANLAB_E2E_LIVE` is set and `HMANLAB_PROVIDERS_ANTHROPIC_API_KEY`
/// is configured.
#[tokio::test]
async fn test_live_agent_run() {
    if !is_live_enabled() {
        eprintln!("Skipping live agent test (HMANLAB_E2E_LIVE not set)");
        return;
    }

    let api_key = match std::env::var("HMANLAB_PROVIDERS_ANTHROPIC_API_KEY") {
    ...
    let provider = hmanlab::providers::ClaudeProvider::new(&api_key);
