...
impl LLMProvider for AlwaysFailProvider {
    ...
    ) -> hmanlab::error::Result<LLMResponse> {
...
async fn test_session_full_conversation() {
    ...
    assert_eq!(loaded.messages[0].role, hmanlab::session::Role::System);
    assert_eq!(loaded.messages[1].role, hmanlab::session::Role::User);
    assert_eq!(loaded.messages[2].role, hmanlab::session::Role::Assistant);
    assert_eq!(loaded.messages[3].role, hmanlab::session::Role::User);
}
...
async fn test_session_with_tool_calls() {
    ...
    let tool_call =
        hmanlab::session::ToolCall::new("call_1", "echo", r#"{"message": "Hello World"}"#);
...
fn test_skills_loader_workspace_override_and_filter_unavailable() {
    ...
    std::fs::write(
        ...
        "---\nname: needs_env\ndescription: Needs env var\nmetadata: {\"hmanlab\":{\"requires\":{\"env\":[\"HMANLAB_INTEGRATION_TEST_MISSING_ENV_2B9F2E16\"]}}}\n---\n# Needs Env",
    )
    .unwrap();
...
async fn test_tool_call_flow() {
    ...
    let tool_call =
        hmanlab::session::ToolCall::new("call_001", "echo", r#"{"message": "Test message"}"#);
    session.add_message(Message::assistant_with_tools(
...
async fn test_runtime_factory_native() {
    use hmanlab::config::RuntimeConfig;
    use hmanlab::runtime::create_runtime;
...
async fn test_cron_scheduling_dispatches_message() {
    use chrono::Utc;
    use hmanlab::cron::{CronPayload, CronSchedule, CronService, OnMiss};
...
async fn test_available_runtimes_includes_native() {
    use hmanlab::runtime::available_runtimes;
...
async fn test_shell_tool_with_native_runtime() {
    use hmanlab::runtime::NativeRuntime;
    use hmanlab::tools::shell::ShellTool;
    use hmanlab::tools::{Tool, ToolContext};
    use std::sync::Arc;
...
async fn test_config_runtime_serialization() {
    use hmanlab::config::{RuntimeConfig, RuntimeType};
...
fn test_container_agent_config_deserialization() {
    ...
    let config: Config = serde_json::from_str(json).unwrap();
    assert_eq!(config.container_agent.image, "hmanlab:custom");
...
fn test_container_agent_config_defaults() {
    let config = Config::default();
    assert_eq!(config.container_agent.image, "hmanlab:latest");
...
fn test_ipc_response_markers() {
    use hmanlab::gateway::{parse_marked_response, AgentResponse, AgentResult};
...
fn test_ipc_parse_with_noisy_stdout() {
    use hmanlab::gateway::{parse_marked_response, AgentResponse};
...
fn test_ipc_parse_missing_markers_returns_none() {
    use hmanlab::gateway::parse_marked_response;
...
fn test_container_agent_proxy_creation() {
    use hmanlab::gateway::{ContainerAgentProxy, ResolvedBackend};
...
fn test_agent_request_serialization() {
    use hmanlab::gateway::AgentRequest;
...
async fn test_delegate_tool_recursion_blocking() {
    use hmanlab::providers::ClaudeProvider;
    use hmanlab::tools::delegate::DelegateTool;

    let config = Config::default();
    let bus = Arc::new(MessageBus::new());
    let provider: Arc<dyn hmanlab::providers::LLMProvider> =
...
async fn test_agent_loop_streaming_accessors() {
    ...
    let session_manager = SessionManager::new_memory();
    let bus = Arc::new(MessageBus::new());
    let agent = hmanlab::agent::AgentLoop::new(config, session_manager, bus);
