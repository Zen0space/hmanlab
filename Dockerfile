...
#
# Build: docker build -t hmanlab .
# Run:   docker run -v hmanlab-data:/data hmanlab
...
RUN cargo build --release --bin hmanlab
...
RUN apt-get update && apt-get install -y \
    ...
    wget \
    && rm -rf /var/lib/apt/lists/* \
    && useradd -r -s /bin/false -d /data hmanlab

# Copy binary from builder
COPY --from=builder /app/target/release/hmanlab /usr/local/bin/
