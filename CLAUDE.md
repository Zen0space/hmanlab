...
./target/release/hmanlab agent -m "Hello"  # Run agent
./target/release/hmanlab config check      # Validate config
./target/release/hmanlab provider status   # Check providers
```
...
- Embedded `HmanLabAgent` tool calls use the same `kernel::execute_tool()` path as the main agent loop and MCP server, so safety scanning, taint checks, and tool metrics stay aligned across entry points.
- Embedded `HmanLabAgent` also supports per-tool timeout, panic capture, and configurable approval gating via the builder for safer embedded coding-agent execution.
...
Config: `~/.hmanlab/config.json`. Env vars override with pattern `HMANLAB_<SECTION>_<KEY>`.
