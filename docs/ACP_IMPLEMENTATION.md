...
This document describes the design and implementation of the Agent Client Protocol (ACP) support in HmanLab. It is intended for maintainers and contributors.
...
HmanLab implements a **minimal** ACP agent surface:
...
- **stdio** (`"acp"`) — The client launches HmanLab as a subprocess and communicates over stdin (client → agent) and stdout (agent → client) using newline-delimited JSON-RPC 2.0 messages.
...
ACP is implemented as a **normal HmanLab channel** (the `Channel` trait). There are two entry points:

1. **`hmanlab gateway`** — ACP stdio is **not** registered here; the gateway's stdin is the terminal, not an ACP client. To expose ACP in gateway mode, set `channels.acp.http.enabled = true`, which registers the `acp_http` channel alongside Telegram, Discord, etc. on the shared `MessageBus`.
2. **`hmanlab acp`** — Standalone subcommand that starts just the ACP stdio agent without the full gateway. Calls `AcpChannel::run_stdio()`, which blocks until stdin closes. This is the entry point used by `acpx` and other ACP clients that spawn the agent as a subprocess.
...
- **Session ID:** ACP’s `sessionId` is generated by the agent in `session/new` (format: `acp_<uuid>`). The same value is used as HmanLab’s **chat ID** for the lifetime of that session, so the agent loop and all routing use one identifier.
...
- **Response fields (schema-authoritative):**
  ...
  - `agentInfo.name: "hmanlab"`, `agentInfo.version: "<crate version>"` — both required strings per schema; `agentInfo.title` is optional
  - `authMethods: []`
...
|-------|------|---------|-------------|
| `enabled` | bool | false | **No-op in gateway mode.** Accepted for backward compatibility; `hmanlab acp` always starts ACP stdio regardless of this flag. |
...
`channels.acp.enabled` is not required for HTTP. For `hmanlab acp` (stdio), no config flag is needed — just run the subcommand. `allow_from` and `deny_by_default` apply to both transports.
...
|----------|-------------|
| `HMANLAB_CHANNELS_ACP_HTTP_ENABLED` | `channels.acp.http.enabled` |
| `HMANLAB_CHANNELS_ACP_HTTP_PORT` | `channels.acp.http.port` |
| `HMANLAB_CHANNELS_ACP_HTTP_BIND` | `channels.acp.http.bind` |
| `HMANLAB_CHANNELS_ACP_HTTP_AUTH_TOKEN` | `channels.acp.http.auth_token` |
...
- **Entry points:** Two ways to run the loop:
  ...
  - `AcpChannel::run_stdio()` — runs the loop directly on the caller's task and blocks until stdin closes. This is the only production entry point, used by `hmanlab acp` so the standalone subprocess stays alive for the full client session.
...
- **loadSession:** If the spec stabilizes session loading, implement optional persistence of session state (e.g. in `~/.hmanlab`) and advertise `loadSession: true` when enabled; handle client requests to restore a session.
...
- **MCP over ACP:** If the client advertises MCP servers in `session/new` or a dedicated method, consider wiring those into HmanLab’s MCP client or exposing them to the agent (with appropriate isolation and policy).
...
These tests spawn a real `hmanlab acp` subprocess and communicate over stdin/stdout using raw JSON-RPC. No LLM calls are made by the wire-protocol tests (they validate protocol-level behavior before prompts reach the agent loop). The four `acpx` end-to-end tests require a live API key and are gated by `HMANLAB_E2E_LIVE=1`.
...
Nextest is configured in `.config/nextest.toml` to set `threads-required = 4` for all tests in this binary, which limits the number of concurrent `hmanlab acp` subprocesses without affecting parallelism for other test binaries.
...
**`acpx` end-to-end tests (require `HMANLAB_E2E_LIVE=1`, 4 tests):**
...
- HmanLab channel layer: `src/channels/mod.rs`, `src/channels/factory.rs`
- HmanLab bus: `src/bus/mod.rs`, `src/bus/message.rs`
