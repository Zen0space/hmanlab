# HmanLab Performance Guide

This document covers performance characteristics, benchmarks, and optimization tips for HmanLab.
...
# 512MB RAM, 1 vCPU
hmanlab gateway --config production.toml
```
...
```bash
# Memory usage
ps -o rss= -p $(pgrep hmanlab)

# CPU usage
top -pid $(pgrep hmanlab)

# Open connections
lsof -p $(pgrep hmanlab) | grep ESTABLISHED | wc -l
```
...
```bash
RUST_LOG=info hmanlab gateway 2>&1 | jq '. | {timestamp, level, fields}'
```
...
# Run with profiling
CARGO_PROFILE_RELEASE_DEBUG=true cargo flamegraph --bin hmanlab
```
...
DHAT_HEAP_PROFILING=1 ./target/debug/hmanlab
```
