...
# Update version + sha256 in qhkm/homebrew-tap Formula/hmanlab.rb
```

### 5. Verify Install Paths

```bash
hmanlab update                    # Self-update
brew upgrade hmanlab              # Homebrew
cargo install hmanlab --version $VERSION --dry-run  # crates.io
```
...
| HmanLab (old) | Burst then silence | Never again |
| HmanLab (new) | Ship when ready, daily minimum | This is the way |
...
| 2026-03-26 | Hotfix = drop everything | Users on `hmanlab update` shouldn't wait for anything when security is involved. |
