...
Run multiple HmanLab tenants on a single VPS using Docker. Each tenant gets complete isolation with zero application code changes.

## Why Container-Per-Tenant

HmanLab is ~4MB binary, ~6MB RSS idle. Docker provides process, filesystem, and network isolation out of the box. No multi-tenant code needed.
...
docker volume rm hmanlab_<name>-data
rm -rf tenants/<name>
```
...
#!/bin/bash
# backup-tenants.sh
BACKUP_DIR="/backups/hmanlab/$(date +%Y-%m-%d)"
mkdir -p "$BACKUP_DIR"

for volume in $(docker volume ls -q | grep -E "^hmanlab_.*-data$"); do
    tenant=$(echo "$volume" | sed 's/hmanlab_\(.*\)-data/\1/')
...
```
0 3 * * * /opt/hmanlab/backup-tenants.sh
```
...
- `HMANLAB_HEALTH_PORT=9090` for liveness/readiness endpoints
- `com.hmanlab.tenant/version/env` container labels for ops filtering
- Docker healthcheck via `wget` to `/healthz`

You can override the version and environment labels:

```bash
HMANLAB_VERSION=0.2.0 HMANLAB_ENV=staging ./scripts/generate-compose.sh > docker-compose.multi-tenant.yml
```
...
- Each container runs as non-root user (`hmanlab`)
