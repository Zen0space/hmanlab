...
**Composed tools** (`composed.rs`): `CreateToolTool` (create/list/delete/run), `ComposedTool` (interpolates `{{param}}` placeholders). Stored at `~/.hmanlab/composed_tools.json`.
...
- `shell.rs` — regex blocklist + optional allowlist (Off/Warn/Strict); blocks `.hmanlab/config.json` exfiltration
...
- `LongTermMemory` — KV store at `~/.hmanlab/memory/longterm.json` with categories, tags, access tracking, injection guard
...
- **Deps** (`src/deps/`): `HasDependencies` trait, `DepKind` (Binary/Docker/Npm/Pip), registry at `~/.hmanlab/deps/registry.json`
...
| Path | Purpose |
|------|---------|
| `~/.hmanlab/config.json` | Main configuration |
| `~/.hmanlab/memory/longterm.json` | Long-term memory store |
| `~/.hmanlab/composed_tools.json` | User-created composed tools |
| `~/.hmanlab/deps/registry.json` | Installed dependency tracking |
| `~/.hmanlab/skills/<name>/SKILL.md` | Skill definitions |
