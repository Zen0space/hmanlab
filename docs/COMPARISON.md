# HmanLab vs OpenClaw vs PicoClaw vs NanoClaw vs ZeroClaw
...
| | **OpenClaw** | **PicoClaw** | **NanoClaw** | **ZeroClaw** | **HmanLab** |
|---|---|---|---|---|---|
...
| Capability | OpenClaw | PicoClaw | NanoClaw | ZeroClaw | HmanLab |
|---|---|---|---|---|---|
...
| Channel | OpenClaw | PicoClaw | NanoClaw | ZeroClaw | HmanLab |
|---|---|---|---|---|---|
...
OpenClaw leads on channel breadth (14+32 extensions). PicoClaw covers Chinese platforms. NanoClaw focuses on WhatsApp. ZeroClaw has 7 channels including iMessage and Matrix. HmanLab has 9 channels (Telegram, Slack, Discord, WhatsApp, WhatsApp Cloud, Lark, Email, Webhook, Serial).

## Security

| Feature | OpenClaw | PicoClaw | NanoClaw | ZeroClaw | HmanLab |
|---|---|---|---|---|---|
...
ZeroClaw and HmanLab take different security approaches. ZeroClaw focuses on access control — gateway pairing, sender allowlists, secret encryption, autonomy levels. HmanLab focuses on content security — prompt injection detection, leak scanning, SSRF prevention, policy engine — and now also has secret encryption at rest (XChaCha20-Poly1305 + Argon2id) and deny-by-default sender allowlists. Both block sensitive filesystem paths.
...
ZeroClaw has the strongest memory search after OpenClaw — hybrid SQLite FTS5 (BM25 scoring) + vector embeddings with cosine similarity, all built-in with no external dependencies. HmanLab uses simpler text matching but adds token budgeting and context compaction.

## Architecture

| Aspect | OpenClaw | PicoClaw | NanoClaw | ZeroClaw | HmanLab |
|---|---|---|---|---|---|
...
| Embedded / IoT devices | **PicoClaw** or **HmanLab** | PicoClaw: I2C/SPI on $10 SBCs. HmanLab: ESP32/RPi/Arduino/Nucleo with GPIO, I2C, NVS |
...
| Secrets encryption at rest | **ZeroClaw** or **HmanLab** | ZeroClaw: ChaCha20-Poly1305. HmanLab: XChaCha20-Poly1305 + Argon2id |
| Tunnel support (Cloudflare/ngrok) | **ZeroClaw** or **HmanLab** | ZeroClaw: 4 options. HmanLab: Cloudflare, ngrok, Tailscale |
| Security-sensitive deployment | **HmanLab** | Multi-layer safety, container isolation, leak detection |
| Resource-constrained server | **HmanLab** or **ZeroClaw** | Both Rust, both <5MB binary |
| Multi-tenant hosting | **HmanLab** | ~6MB per tenant, container isolation per request |
...
| Batch processing / automation | **HmanLab** | Batch mode, routines, cron, agent templates |
| Cost-conscious API usage | **HmanLab** | Token budget, cost tracking, retry + fallback |
| Browser automation | **OpenClaw** or **ZeroClaw** | OpenClaw: CDP. ZeroClaw: agent-browser |
| Hardware sensor integration | **PicoClaw** or **HmanLab** | PicoClaw: I2C, SPI. HmanLab: GPIO, I2C, NVS for 4 board families |
| Agent swarms | **NanoClaw** or **HmanLab** | NanoClaw: Claude Agent Teams. HmanLab: DelegateTool (parallel fan-out + sequential scratchpad) |
...
**HmanLab** is the most secure and operationally complete — multi-layer safety, per-command container isolation, secret encryption at rest (XChaCha20-Poly1305 + Argon2id), sender allowlists, tunnel support (Cloudflare/ngrok/Tailscale), token budgeting, cost tracking, batch mode, hardware peripherals (ESP32/RPi/Arduino/Nucleo), and parallel agent swarms in a 4MB binary. 9 channels including Serial for embedded devices. The tradeoff is no companion apps.

All five are open source, self-hosted, and built for developers who want to own their AI assistant. The right choice depends on your priorities: features (OpenClaw), portability (PicoClaw), simplicity (NanoClaw), provider flexibility (ZeroClaw), or security and efficiency (HmanLab).
